...
if (!$cnn) {
  ...
  while ($fila2 = mysqli_stmt_fetch($sqlphoto)) {
    if ($foto_uno == 0) {
      echo "<div class='col-lg-4 col-md-6 col-sm-12 col-xs-12'>
        <div style='padding:3px;'>
          <div class='card'>
            <a href='$url_galeria$nameimg' class='text-decoration-none' data-toggle='lightbox' data-gallery='gallery$ID'><img src='";
            if(isset($nameimg)){
              echo $url_galeria.$nameimg;
            }else{
              echo $url_photo_none;
            }
            echo"' class='gallery-menu' alt='imagen'></a>
            <div class='card-body'>
              <div class='row'>
    ...
    $foto_uno++;
    if ($foto_uno > 1) {
      echo "<div data-toggle='lightbox' data-gallery='gallery$ID' data-remote='$url_galeria$nameimg'></div>";
    }
  }
